...
class Clock {
 ...
 public:
  ...
  void start(){
	// 获取当前时间，real_time_start_返回时间
    gettimeofday(&real_time_start_, NULL);
	// clock返回从“程序进程开始”到“调用该函数”间的CPU计时单元数
    cpu_start_ = clock();
  }
  ...
  void takeTime(){
	...
	// CLOCKS_PER_SEC一秒钟有多少计时单元，kSecondsToMiliseconds秒转毫秒
...
// vector<Pose, Eigen::aligned_allocator<Pose> >
typedef Aligned<std::vector, Pose> PoseVector;
// vector<RelativePose, Eigen::aligned_allocator<RelativePose> >
...
static void correctTransformationMatrix(
  ...
  // 检验rigid_transformation的旋转阵是否为正交阵
  if (!rigid_transformation->checkParameters(*transformation_matrix)) {
	...
	// 将rigid_transformation旋转阵转换为正交阵
...
static void writeCSV(const StringMatrix& string_matrix, const std::string& filename) {
  // CEHCK_GE涉及到glog中的一系列宏，大于等于判断
  // 如果true则返回null，false则返回有明确指示的string*，并输出日志，程序退出
