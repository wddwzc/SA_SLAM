...
void LaserSlamWorker::init(
  ...
  // work_id即为对应的robot编号，单机工作时，为0
  worker_id_ = worker_id;

  // Get the LaserTrack object from the IncrementalEstimator.
  // 获取IncrementalEstimator中的lasertrack对象
...
void LaserSlamWorker::scanCallback(const sensor_msgs::PointCloud2& cloud_msg_in) {
  ...
  if (!lock_scan_callback_) {
	// warForTransform  参数4: 等待时间
	// 此处获取的tf可以认为就是odometry输出的位姿
    if (tf_listener_.waitForTransform(params_.odom_frame, params_.sensor_frame,
      ...
      if (process_scan) {
        ...
        gtsam::Values result;
		// is_prior在trajectory_为空时被赋值为true，否则为false
		// 初始的时候采用默认值
        if (is_prior) {
		...
		// 去除地面（参数中remove_ground_from_local_map为false）
...
// 在此过程中，类成员保存一些中间值
// local_map_ 直接取圆柱内部点
// local_map_filtered_ 体素滤波后，再取圆柱内部点
// distant_map_ 体素滤波后，再取圆柱外部点
void LaserSlamWorker::getFilteredMap(PointCloud* filtered_map) {
  ...
  if (params_.separate_distant_map) {
	...
	// 把离 robot 较远的点存在 distant_map_ 中，便于单独处理
    clock.start();
...
// 清空local_map_和local_map_filtered_
...
// 输出路径尾部一定时间内的点到CSV（head_duration_ns控制时长）
