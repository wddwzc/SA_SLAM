...
bool SegMatchWorker::processLocalMap(
	// 即typedef LocalMap<PclPoint, MapPoint> LocalMapT 与维护的LocalMap类型相同
  ...
  if ((params_.localize && target_cloud_loaded_) || params_.close_loops) {
    ...
    if (last_segmented_poses_.empty()) {
	  ...
	  // 搜索last_segmented_poses_中与当前id对应的元素
    ...
    if (params_.export_segments_and_matches) {
	  // 返回当前的源点云，添加到segments_database_中
...
// 发布目标点云沿Z轴下移 distance_to_lower_target_cloud_for_viz_m
...
// 源点云发布时沿Z轴上移 distance_to_lower_target_cloud_for_viz_m
